...
function Login() {
  ...
  const loginValidation = async (e?: React.MouseEvent<HTMLButtonElement>) => {
    e?.preventDefault();
    if (!email || !password) {
      toast.error(<div style={{ color: "red" }}>Existen campos vacios</div>, {
        position: "top-right",
      });
      return;
    }
    ...
    if (
      ...
      toast.error(
        <div style={{ color: "red" }}>Dirección de correo no valida</div>,
        { position: "top-right" }
      );
      return;
    }
    ...
    try {
      ...
      if (response.data.login) {
        ...
        if (response.data.requerir_cambio_password) {
          toast(
            <div style={{ color: "orange" }}>{response.data.mensaje}</div>,
            { position: "top-right" }
          );
        ...
        toast.dismiss(loadingToast);
        toast.error(
          <div style={{ color: "red" }}>{response.data.mensaje}</div>,
          { position: "top-right" }
        );
      }
    } catch (error) {
    ...
    >
      <div className="bg-white  shadow-2xl p-8 w-full max-w-md flex flex-col items-center gap-6 rounded">
        ...
        <form className="w-full flex flex-col ">
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full px-4 py-2 border  shadow-sm focus:outline-none focus:ring-2 focus:ring-[var(--agua)]"
        ...
        <p className="text-sm text-gray-600">
          ...
          <Link
            to="/Register"
            className="text-[var(--bosque)] hover:underline font-semibold"
          >
            Regístrate aquí
          </Link>
        </p>
